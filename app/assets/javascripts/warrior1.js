function Warrior1()
{
    ...
    this.ChooseBaseAction = function()
    {
        ...
        else if(tick - this.warplan.startTick < 160)
        {
        ...
        else if(this.myUnitCounts[UnitTypeEnum.TANK] < 2 && this.canAfford(UnitTypeEnum.TANK) && this.totalmyUnits > 25)
        {
            ...
            && tick - this.warplan.startTick > 200)
        {
        ...
        for(var i = 0; i < this.warplan.myUnits.length; i++)
        {
            ...
            for(var j = 0; j < this.warplan.enemyUnits.length; j++)
            {
                ...
                if(distance < closestDistance)
                {
                    ...
                    //if this guy is closer than the current closest and 
                    //(I'm not targeting anyone or the current target isn't a sniper)
                    //choose this guy.  That means I'll target the closest guy, assuming there's no sniper in range.  Otherwise, 
                    //I'll target the sniper
                    closestUnit = this.warplan.enemyUnits[j];
                    closestDistance = distance;
                }
            ...
            if(closestUnit == null)
            {
                //go to my part way between my own base and enemy base and hold there - gattling is a defensive measure!
        ...
        for(var i = 0; i < this.warplan.myUnits.length; i++)
        {
            ...
            for(var j = 0; j < this.warplan.enemyUnits.length; j++)
            {
                distance = Math.pow(
                             ...
                             Math.pow(this.warplan.myUnits[i].y - this.warplan.enemyUnits[j].y, 2), 
                             0.5);
                //make sure nobody can move into range and then fire next turn
                buffer = 4;
                ...
                if(this.warplan.myUnits[i].unitType != UnitTypeEnum.GUNNER || true)
                {
                    ...
                    //since I can't catch them anyways and they can be used to harass me
                    //I want gunners to chase them, since they are fast and have range, and can trap
            ...
            {
                if(window.tick - this.warplan.startTick < this.initialDelay && closestUnit == null)
                {
                 continue;
                }
            }
            ...
            if(closestUnit == null)
            {
                if(this.warplan.myUnits[i].unitType == UnitTypeEnum.SCOUT)
                {
                    ...
                    else
                    {
                        ...
                        this.warplan.myUnits[i].moveDirection = new Array(x, y);  
                        this.warplan.myUnits[i].moveSpeed = this.warplan.myUnits[i].speed; 
                        //once somebody is chasing, remove it from the list so
                        //we don't waste time competing for the same resources
                        //this.warplan.resources.splice(closestResourceIndex, 1); 
                    }
            ...
            if(this.warplan.myUnits[i].moveDirection && closestDistance > this.warplan.myUnits[i].range + 8)
            {
               ...
               //closeCount should never be 0, since we're included in the closeCount
               centerx = centerx / closeCount;
               centery = centery / closeCount;
               if(this.warplan.myUnits[i].x != centerx || this.warplan.myUnits[i].y != centery)
               {
